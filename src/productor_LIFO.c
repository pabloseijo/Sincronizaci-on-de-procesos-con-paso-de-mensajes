...
unsigned int prio1, prio2=0;

char producir_item(int iter){
	printf("Produjo la %c con prioridad %d\n", 'a'+(iter % MAX_BUFFER), prio2);
...
void productor(){
	...
	for(int i=0; i < DATOS_A_PRODUCIR; i++){
		...
		//Recibir hueco vacio
		if(mq_receive(almacen1, &empty, sizeof(char), &prio1) == -1){
			exit(0);
		}
		...
		//Enviar item
		if(mq_send(almacen2, &item, sizeof(char), prio2) == -1){
			exit(0);
		}
		prio2++;
